				  ...
				  <tbody>
				  	<?php 
						...
						foreach ($cur as $result) {
				  		...
				  		echo '<td>'. $result->shop_name.'</td>';				  		if($result->status=='active'){				  				echo '<td> <a href="#"  class="btn btn-success btn-xs" disabled>Active</a>								<a href="controller.php?action=edit&id='.$result->owner_id.'&name='.$result->name.'&actions=deac" class="btn btn-danger btn-xs">Deactivate</a>				  				</td>';			  	 		}elseif($result->status=='inactive'){				  	 			echo '<td><a href="#"  class="btn btn-danger btn-xs" disabled>Inactive</a>								<a href="controller.php?action=edit&id='.$result->owner_id.'&name='.$result->name.'&actions=active" class="btn btn-danger btn-xs">Activate</a>								</td>';			  	 		}				  		echo '</tr>';
				  	}
				  	?>
				  </tbody>
