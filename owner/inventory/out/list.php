...
<span id="printout">
	<div class="col-md-12" >
	<div class="page-header" style="text-align:center;" ><h1>List of Phased Out Products Available</h1></div>
			  ...
			  <tbody>
					...
					$sid = $_SESSION['shop_id'];
				  		$query = "SELECT * FROM `tblpromopro` pr , `tblproduct` p , `tblcategory` c 
           					 WHERE pr.`PROID`=p.`PROID` AND  p.`CATEGID` = c.`CATEGID` AND p.`PROSTATS` = 'Discontinued' AND  p.`shop_id` = '$sid'";
				  		$mydb->setQuery($query);
