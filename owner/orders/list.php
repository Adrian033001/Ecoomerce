...
<div class="container">
	<?php
 	...
 	<div class="row">
       	 <div class="col-lg-12">
            <h1 class="page-header">List of Orders <a href="../../index.php" target="_blank" class="btn btn-primary btn-xs  ">  <i class="fa fa-plus-circle fw-fa"></i> Add Order</a></h1>
       		</div>
        	<!-- /.col-lg-12 -->
   		 </div>
                  ...
                  <table id="example" class="table  table-striped table-bordered table-hover"  style="font-size:12px" cellspacing="0">
					...
					$sid=$_SESSION['shop_id'];
						...
						$query = "SELECT  DISTINCT * FROM `tblsummary` s ,`tblcustomer` c , `tblorder` ord , `tblproduct` p
				  				WHERE   s.`CUSTOMERID`=c.`CUSTOMERID` AND s.`ORDEREDNUM`=ord.`ORDEREDNUM` AND ord.`PROID`=p.`PROID` AND p.`shop_id` = '$sid' GROUP BY ord.`ORDEREDNUM` ORDER BY s.`ORDEREDNUM` DESC ";
						...
						echo '<tr>';
				  		...
				  		if($result->ORDEREDSTATS=='Pending'){
			  	 			 ...
			  	 			 echo '<td> <a href="#"  class="btn btn-success btn-xs" disabled>To be delivered</a></</td>';				

			

			  	 		}elseif($result->ORDEREDSTATS=='Delivery'){

			  	 			 echo '<td> <a href="#"  class="btn btn-success btn-xs" disabled>Out for delivery</a></td>';				
