...
function doInsert(){
	if(isset($_POST['PROID'])){
						...
						$res = $autonumber->set_autonumber('');
  				 	 	...
  				 	 	$respro = New respro(); 
						$respro->shop_id 		= $_SESSION['sshop_id'];
						...
						$prod = New Product();
						$prod->qtyplus($_POST['PROID'],$_POST['PROQTY']); 
						$PROD = $_POST['PROID'];
						$qty1 		= $_POST['PROQTY'];
...
function doInsert2(){
	if(isset($_POST['selector'])==''){
			message("Select the products first before you add replenishment!","error");
			redirect('../index.php');
			}else{
		$id = $_POST['selector'];
		$qty = $_POST['PROQTY'];
			...
			for($i=0;$i<$key;$i++){
						...
						$ex->PROID 		= $id[$i]; 
						$ex->PROID 		= $id[$i]; 
						$ex->qty 		= $_POST['PROQTY'][$i]; 
						$ex->create();
												
						
						$prod = New Product();
						$prod->qtydeduct($id[$i],$_POST['PROQTY'][$i]); 
						$PROD = $id[$i];
						$qty1 		= $_POST['PROQTY'][$i];
						
						message("New Expired item added successfully!", "success");
						redirect("../index.php");
						}
			}
