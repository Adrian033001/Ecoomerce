...
function doInsert(){
	if(isset($_POST['save'])){
						...
						$res = $autonumber->set_autonumber('');
  				 	 	...
  				 	 	$respro = New respro(); 
  		//		 	 	$respro->restock_id 		= $res->AUTO;
						$respro->res_id 		= $_POST['res_id'];
						$respro->PROID 		= $_POST['PROID']; 
						$respro->qty 		= $_POST['PROQTY']; 
						
						$respro->create();
						$PROD = $_POST['PROID'];
						$qty1 		= $_POST['PROQTY']; 
						/*$product = New Product();
						$product->PRODID = $_POST['PROID']; 
						$product->qty = $_POST['PROQTY']; 
						$product->replenish();*/
						//S$sql = "SELECT * FROM `tblproduct` WHERE PROID = $PROD"
						//if(!$result-> num_rows = 1){
						//$query = "INSERT into `tblproduct` (PROQTY)
						//	VALUES ($PROQTY = $PROQTY + $qty1) WHERE PROID = $PROD";
						//$sql = "UPDATE tblproduct
						//SET PROQTY = PROQTY + 2 WHERE PROID = '201745'";
						//}
						
	//$mydb->setQuery("SELECT * FROM `tblpro	duct` where PROID = " $PROD);
	//$cur  = $mydb->loadResultList();
	//foreach ($cur as $results){
		//echo 'PROID';
	//}
						message("New Replenishment created successfully!", "success");
						redirect("../index.php");
						}
