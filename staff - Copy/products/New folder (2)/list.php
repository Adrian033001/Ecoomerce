        	...
        	<!-- /.col-lg-12 -->
			  ...
			  <tbody>
				    ...
				    echo '<td style="padding:0px;">
				  		...
				  		echo '<td><a title="edit" href="'.web_root.'staff/products/index.php?view=edit&id='.$result->PROID.'"><i class="fa fa-pencil "></i>'.$result->PRODNAME.'</a></td>';
				  		
				  		echo '<td>'. $result->PRODESC.'</td>'; 
				  		echo '<td> &#8369 '.  number_format($result->PROPRICE,2).'</td>';

				  		echo '<td width="4%">'. $result->PROQTY.'</td>';												if($result->PROSTATS == 'Available'){							echo '<td>								<a href="#"  class="btn btn-success btn-xs" disabled>'.$result->PROSTATS.'</a></br></br>								<a href="controller.php?action=editStatus&id='.$result->PROID.'&actions=Discontinued"  class="btn btn-danger btn-xs">Phased Out</a>								</td>';						}elseif($result->PROSTATS == 'Discontinued'){			  	 			 echo '<td> 								<a href="#"  class="btn btn-danger btn-xs" disabled>Discontinued</a></br></br>								<a href="controller.php?action=editStatus&id='.$result->PROID.'&actions=Available"  class="btn btn-success btn-xs">Continued</a></td>';							  	 		}
