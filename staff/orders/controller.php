...
function doInsert(){
	...
	function doEdit(){
		...
		}elseif ($_GET['actions']=='pack') {
							# code...
				$status	= 'Prepaired';	
				$remarks ='Your order has  been packed for delivery.';
				$delivered = Date('Y-m-d');
				 
		}elseif ($_GET['actions']=='delivery') {
							# code...
				$status	= 'Delivery';	
  	...
  	// 		$res = $customer->single_customer($_GET['customerid']); 
	        ...
	        $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$cur->PHONE."','','FROM Bachelor of Science in Information Technology : Your order has been '".$status. "'. The amount is '".$cur->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
			...
			foreach ($cur as $result) {
	  		 ...
	  		 $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$result->OWNERPHONE."','','FROM Bachelor of Science in Information Technology : Your  product has been ordered'. The amount is '".$result->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
