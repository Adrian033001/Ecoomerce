...
 function editStatus(){ 		if ($_GET['actions']=='Discontinued'){		$product = New Product();		$product->PROSTATS	= 'Discontinued';		$product->up_date	= date('Y-m-d');				$product->update($_GET['id']);	}elseif($_GET['actions']=='Available'){		$product = New Product();		$product->PROSTATS	= 'Available';		$product->up_date	= date('Y-m-d');		$product->update($_GET['id']);	}	redirect("index.php"); } 
...
function doInsert(){
	if(isset($_POST['save'])){
			...
			$myfile =$_FILES['image']['name'];
		 	$location="../../../staff/products/uploaded_photos/".$myfile;
			...
			if ($image_size==FALSE || $type=='video/wmv') {
				...
				redirect("index.php?view=add");
					...
					redirect('index.php?view=add');
						...
						$product->shop_id		= $_SESSION['sshop_id']; 
						$product->PROSTATS		= 'Available';
						$product->create();
						// }
