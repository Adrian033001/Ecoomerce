...
switch ($action) {
	case 'add' :		$res = $_POST['restock_id'];				$mydb->setQuery("SELECT * FROM `tblrestock` r  						  WHERE   r.`restock_id` = '$res'");                          $cur = $mydb->loadResultList();                        foreach ($cur as $result) {					$reqty = $result->qty;					$prod = $result->PROID;
	doInsert($reqty, $prod);						}
	break;
...
function doInsert($rqty, $pro){
	if(isset($_POST['save'])){		$pqty = $_POST['qty'];echo '<script>console.log("'.$pro .'"); </script>';                          
		if($pqty <= $rqty){						$autonumber = New Autonumber();						$res = $autonumber->set_autonumber('');  				 	 	$damaged = New damage();												$damaged->res_id = $_POST['res_id'];						$damaged->restock_id = $_POST['restock_id'];						$damaged->dam_id = $_POST['dam_id'];						$damaged->dam_qty = $_POST['qty'];												$damaged->create();												$prod = New Product();						//$value1		= $_POST['PROQTY']; 						//$id1 		= $_POST['PROID']; 						$prod->qtydeduct($pro,$_POST['qty']);						$res = New respro();						//$value1		= $_POST['PROQTY']; 						//$id1 		= $_POST['PROID']; 						$res->qtydeduct($_POST['restock_id'],$_POST['qty']);												message("Damaged Item added successfully!", "success");						redirect("../index.php");			}else{			echo '<script>alert("Damaged Item Quantity must be LESSER OR EQUAL to Replenishment Quantity")</script>';				message("SDSADAS", "Error");					redirect("../index.php");									
			}	}}
